...
export default function TreeView() {
  ...
  const handleNewMember = (newMember) => {
    ...
    fetchPeople(); // Refresh the data automatically
  ...
  useWebSocket(handleNewMember, handleMemberDeleted);
  ...
  useEffect(() => {
    const handleNewMemberNotification = (event) => {
      ...
      // Example: showToast(`${name} joined as ${relation}`);
    };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-rose-50 via-white to-amber-50 overflow-hidden">
      ...
      {/* ✅ Optional: Real-time notification badge */}
      <div className="fixed top-20 right-6 z-50">
        <div className="bg-green-500 text-white px-3 py-1 rounded-full text-sm font-medium shadow-lg animate-pulse hidden" id="newMemberNotification">
          New member joined!
        </div>
      </div>
    </div>
