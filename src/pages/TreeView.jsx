    ...
    <div className="border rounded p-3 bg-white shadow flex justify-between items-center">
      <div className="flex items-center gap-3">
        <img
          src={
            person.photo_url
              ? `${BACKEND}/${person.photo_url.replace(/^\/?/, '')}`
              : `https://via.placeholder.com/64?text=${person.name?.charAt(0) || "?"}`
          }
          alt={person.name}
          className="w-16 h-16 rounded-full object-cover"
        />
        <div>
          ...
          <div className="text-xs text-gray-500">{person.relation_label || person.relation_code}</div>
          {person.parent_ids && person.parent_ids.length > 0 && (
            <div className="text-xs text-gray-400">
              Parents: {person.parent_ids.join(", ")}
            </div>
          )}
        </div>
      </div>
...
export default function TreeView() {
  ...
  const fetchPeople = async () => {
    try {
      ...
      console.error("Error fetching people:", err);
      alert("Failed to fetch people from backend.");
    }
  };
  ...
  // Group people by side
  const groupedBySide = people.reduce((acc, person) => {
    const side = person.side || "other";
    if (!acc[side]) acc[side] = [];
    acc[side].push(person);
    return acc;
  }, {});
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6 relative">
      <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 gap-6">
            {["groom", "bride", "other"].map(sideKey => (
              <div key={sideKey} className="bg-white p-4 rounded-xl shadow">
                <h3 className="font-semibold mb-4">{sideKey.toUpperCase()}</h3>
                <div className="space-y-3">
                  {(groupedBySide[sideKey] || [])
                    .sort((a, b) => (a.parent_ids?.length || 0) - (b.parent_ids?.length || 0))
                    .map(person => (
                      <PersonCard key={person._id} person={person} onDelete={handleDelete} />
                    ))}
                </div>
              </div>
            ))}
        ...
        <p className="mt-6 text-sm text-gray-600">
          This is a starter visual. Later we can compute exact node placement using parent_ids and draw connectors / PDF export.
        </p>
      </div>
